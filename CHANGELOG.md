...
- `vicia.pending-isolation.v1` clean-source benchmark receipts and `just pending-isolation-smoke` / `just pending-isolation-full`: one shared 1M committed selected-attribute base, unrelated WAL variants through 1M facts, a selected 10K control, fresh-child warmup plus 5/20 raw elapsed/RSS samples, smaps breakdowns, cursor diagnostics, exact count/checksum validation, and strict RSS/latency acceptance gates. HAL7800 evidence at source `aaf32a9` keeps selected pending entries/bytes and visits at zero for every unrelated variant; the selected control records 10K entries, 1,440,000 snapshot bytes, and 10K visits.
