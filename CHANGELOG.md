...
- A5-5 shared Gate E tagged portability/corruption corpus: the same setup produces a native v10 fixture and a real-Chrome BrowserDb fixture; both consumers run both graphs and compare exact tagged current/history/valid-time/Ref/Keyword/retraction/VAET results. Shared mutations prove previous-manifest fallback and hard-error boundaries for slots, manifests, segments, truncation, headers, and unpublished tails. Browser query results now use the same lossless `src/json_value.rs` encoder as session frames. Native tests and all 23 BrowserDb tests run through a repeatable headless-Chrome script, now enforced by CI. The corpus exposed page-local base integrity as the remaining bounded-read storage blocker rather than claiming Gate E complete.
