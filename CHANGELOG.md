...
- Close R2 with the complete Vetch v13 browser-package rollout. Clean Vicia source `b909cf5` generates one package containing the WASM, JS glue, Rust and TypeScript declarations, manifest, licenses, and provenance; the final clean Vetch package commit passes the 77-test real-Chrome format matrix, authority spike/contract/lifecycle, canvas persistence concurrency, the complete Vetch authority suite, TypeScript checking, and the production build. The package version, default v12 writer, explicit maintenance ownership, and Vetch scheduling policy remain unchanged. R3 bounded-memory recompact is next.
