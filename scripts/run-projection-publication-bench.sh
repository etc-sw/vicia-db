...
profile="${1:-smoke}"
output_dir="${2:-target/projection-publication/$profile}"
case "$profile" in
  smoke) facts=10000 ;;
  full) facts=1000000 ;;
  *)
...
cargo run --release --features bench-internals --bin current-projection-tail-bench -- \
  publish "$source_graph" "$published_graph" "$fixture" "$facts" "$profile" >"$receipt"
node scripts/validate-projection-publication-receipt.mjs "$receipt" "$profile"
node scripts/audit-projection-publication-validator.mjs "$receipt" "$profile"
