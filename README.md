...
Published compatibility package: [`@minigraf/browser`](https://www.npmjs.com/package/@minigraf/browser) (IndexedDB-backed, `wasm-pack`). Vetch consumes the current checkout through its local `@vicia-db/browser` package boundary. `BrowserDb.open()` remains the eager compatibility API, including synchronous `exportGraph()`. New authority callers should use `BrowserDb.openPaged()`: after any required v10→v11 migration commits, it loads bounded v11 catalog/manifest metadata and fetches generation-verified fact/index pages on demand. Its matching portability API is `await exportGraphAsync()`, which verifies and serialises the full published image without keeping every page resident. `importGraph()` retains native-compatible previous-manifest recovery; authority cutovers that will reopen through the bounded path should use `await importGraphForPagedAccess(blob)`, which rejects before replacement unless the post-migration image is complete v11 paged authority. Browser and native session queries share one lossless tagged JSON encoding; browser writes return ordered `tx_count` and durability/maintenance fields. `executeAtomic(commands)` publishes a bounded transact/retract command list under one transaction identity and is the mixed-write boundary for event-plus-projection or current-head replacement. Caller-scheduled `runIdleMaintenance()` atomically rebuilds a compact IndexedDB image when delta thresholds are crossed. Keep foreground writer ownership under a Web Lock; run a legacy v10 database's first paged migration, import, full export, and maintenance in a disposable worker that acquires the same lock, reports its outcome, terminates, and lets the caller reopen through `openPaged()`. See [`docs/DURABILITY_AND_CALLER_RULES.md`](docs/DURABILITY_AND_CALLER_RULES.md) and [`docs/MAINTENANCE_API_CONTRACT.md`](docs/MAINTENANCE_API_CONTRACT.md).

WASI build (`wasm32-wasip1`) remains available as [`@minigraf/wasi`](https://www.npmjs.com/package/@minigraf/wasi) and as a GitHub Releases artifact. File format v11 gives native and browser consumers the same generation-bound, page-local base integrity checks, and browser v10 migration is durable before either persistent open API returns. On the recorded 1M-fact Chrome 150 matrix, `openPaged()` starts in a 17.8 ms five-run maximum and the open-plus-six-probe phase adds at most 51.1 MiB of sampled PSS; one-fact writes remain 8.3 ms p95. Legacy migration, import, full verified export, and maintenance are explicit O(total) operations and must run in a disposable worker. Vetch main `1b57689` vendors clean Vicia source `e60a7c2`, uses `openPaged()` for foreground authority handles, and proves Web-Locked disposable migration/import/export/maintenance plus termination/reopen behavior in real Chrome. Its external 1M/1,024-slice Gate D trace measures 2.378 ms append p95, 3.098 ms checkpoint p95, and 173.988 ms for the exact 1,024-card current-space rebuild. Gates D and E are closed on their recorded hosts; the later Gate A removal of the legacy canvas co-authority and packaged Windows WebView2 smoke remain separate.
