...
- [ ] Roll out file format v12 adaptive prefix leaves. The implementation keeps raw leaves when compression loses, uses restart-16 prefix leaves for repeated AEVT/AVET keys, reads v11 without rewriting it, preserves v11 foreground delta checkpoints, and upgrades only through idle COW maintenance. After terminating two 21–24-hour stale Playwright renderer trees, the uncontended clean 1M fill-90 receipt still reduces the image from 301.363 to 269.586 MiB (-10.54%); point and aggregate p50 improve by 6.35% and 9.08%, checkpoint p50 changes by +1.32%, and checkpoint RSS p50 falls 0.625 MiB. Aggregate tail now passes at 106.96%, but rollout remains open because checkpoint p95/p50 is 120.62%, above the 115% gate.
...
- Attribute and stabilize the fill-90 v12 checkpoint tail without relaxing the 115% gate. The uncontended receipt shows the slow samples increasing EAVT/AEVT collect-sort and several index builds together rather than isolating one prefix-codec phase. Run the real-browser WASM suite and replace Vetch's complete browser package only after checkpoint evidence passes; do not copy only the `.wasm` binary.
