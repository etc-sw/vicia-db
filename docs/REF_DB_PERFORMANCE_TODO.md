...
- [x] Verify unrelated pending attributes do not increase selected-attribute memory. The clean `vicia.pending-isolation.v1` full receipt keeps selected pending entries/bytes and pending visits at zero through 1M unrelated WAL facts; query RSS delta differs from zero-pending by at most 0.875 MiB.
...
- Compare retained RSS after the first and twentieth repeated aggregate, separating live cursor/cache state from allocator retention. The pending-isolation receipt already records the raw per-run RSS and smaps breakdown needed for that analysis.
