...
- Stabilize the repeated point-read measurement contract without relaxing its regression threshold or selecting a lucky retry; the absolute p95-p50 gap is only 30 microseconds but prevents an all-gates receipt. Then run the real-browser WASM suite with a compatible `CHROMEDRIVER` and replace Vetch's complete browser package, never only the `.wasm` binary.
