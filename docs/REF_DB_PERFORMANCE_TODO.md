...
- Add an allocation-free AEVT projection decoder that yields only the fields the current-attribute reducer consumes instead of fully deserializing every owned `AevtKey`. Keep the page-backed cursor and restart seek as the durable traversal boundary, and require the same leaf-read receipt to reach aggregate p50 `<= 230 ms` or improve by at least 10% before rerunning storage-layout and real-browser rollout gates. Do not replace Vetch's browser package until that gate passes; when it does, replace the complete package, never only the `.wasm` binary.
